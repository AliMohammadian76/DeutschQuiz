      ...
      <div className="mx-auto max-w-7xl px-5 py-6 sm:px-8 lg:px-10">
        <header className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="rounded-full bg-[#172033] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-700">ورود / ثبت‌نام</button>
          </div>
        </header>
        ...
        <section className="relative mt-14 grid items-center gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="relative z-10">
            ...
            <div className="mt-8 flex flex-wrap gap-4"><button className="rounded-2xl bg-cyan-600 px-6 py-3.5 text-sm font-bold text-white shadow-xl shadow-cyan-200 transition hover:-translate-y-0.5 hover:bg-cyan-700">شروع آزمون رایگان ←</button><button className="rounded-2xl border border-slate-200 bg-white px-6 py-3.5 text-sm font-bold text-slate-700 transition hover:border-slate-300">مشاهده‌ی پیشرفت</button></div>
          ...
          <div className="relative mx-auto w-full max-w-md"><div className="absolute -inset-6 rounded-[3rem] bg-gradient-to-br from-cyan-100 via-white to-violet-100 blur-2xl" /><div className="relative rounded-[2rem] border border-white/80 bg-white/90 p-6 shadow-2xl shadow-slate-200/80 backdrop-blur">
            <div className="flex items-center justify-between"><div><p className="text-xs text-slate-400">پیشرفت این هفته</p><p className="mt-1 text-2xl font-black">۶۸٪</p></div><div className="grid h-12 w-12 place-items-center rounded-2xl bg-cyan-50 text-xl">↗</div></div>
            <div className="mt-6 flex h-32 items-end justify-between gap-2">{[34, 54, 42, 76, 61, 88, 68].map((height, index) => <div key={index} className="flex flex-1 flex-col items-center gap-2"><div className={`w-full rounded-t-lg ${index === 5 ? "bg-cyan-500" : "bg-cyan-100"}`} style={{ height: `${height}%` }} /><span className="text-[10px] text-slate-400">{["ش", "ی", "د", "س", "چ", "پ", "ج"][index]}</span></div>)}</div>
            <div className="mt-5 rounded-2xl bg-slate-50 p-4"><div className="flex items-center justify-between text-xs"><span className="font-bold text-slate-700">Lektion 1</span><span className="font-bold text-cyan-600">در حال پیشرفت</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full w-[68%] rounded-full bg-cyan-500" /></div></div>
          </div></div>
        </section>
        ...
        <section className="mt-20 pb-12"><div className="mb-7 flex items-end justify-between"><div><p className="text-sm font-bold text-cyan-600">انتخاب کن و شروع کن</p><h2 className="mt-2 text-2xl font-black">نوع آزمون را انتخاب کن</h2></div><span className="text-sm text-slate-400">۳ حالت آزمون</span></div>
          <div className="grid gap-5 md:grid-cols-3">{quizModes.map(([title, subtitle, description, icon, color]) => <button key={title} className="group text-right"><div className="h-full rounded-3xl border border-slate-100 bg-white p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/60"><div className={`grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br ${color} text-xl font-black text-white shadow-lg`}>{icon}</div><div className="mt-5 flex items-center gap-2"><h3 className="text-lg font-black">{title}</h3><span className="text-xs text-slate-400">{subtitle}</span></div><p className="mt-3 text-sm leading-7 text-slate-500">{description}</p><div className="mt-6 text-sm font-bold text-cyan-600 transition group-hover:translate-x-[-4px]">شروع کن ←</div></div></button>)}</div>
        </section>
      </div>
    </main>
  );
